...
async function buatPembayaran() {
  ...
  try {
    const res = await fetch(`${global.apikeyorkut}/api/orkut/createpayment?apikey=${global.apiSimpelBot}&amount=${amount}&codeqr=${global.qrisOrderKuota}`);
...
Note : QRIS hanya berlaku selama 5 menit.`;
    ...
    interval = setInterval(async () => {
      const cek = await fetch(`${global.apikeyorkut}/api/orkut/cekstatus?apikey=${global.apiSimpelBot}&merchant=${global.merchantIdOrderKuota}&keyorkut=${global.apiOrderKuota}`);
...
function getSpecByCmd(cmd) {
  const specs = {
    ...
    "unlimited": { ram: "0", disk: "0", cpu: "0", harga: "15000" }
  };
  return specs[cmd];
}
...
async function buatAkunDanServer(username, Obj) {
  ...
  try {
    const buatUser = await fetch(`${global.domain}/api/application/users`, {
      method: "POST",
      headers: {
    ...
    const buatServer = await fetch(`${global.domain}/api/application/servers`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
